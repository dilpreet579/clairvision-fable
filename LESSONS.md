...
- [worker-death-mid-chord-wedges-event](lessons/worker-death-mid-chord-wedges-event.md) — a transient worker OOM/kill at stage 2 wedges the event forever (early-ack loses the chord task, watchdog won't reap PROCESSING, nothing re-launches); size the VM for peak single-worker model RSS (8GB m7i-flex.large) and set acks_late.
